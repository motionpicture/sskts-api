...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const auth = new sskts.auth.GoogleToken(idToken, 'motionpicture', 'teststate', ['https://sskts-api-development.azurewebsites.net/people.profile']);
        const credentials = yield auth.refreshAccessToken();
        debug('credentials:', credentials);
        let profile = yield sskts.service.person.getProfile({
            auth: auth,
            personId: 'me'
        });
        ...
        profile = yield sskts.service.person.getProfile({
            auth: auth,
            personId: 'me'
        });
        debug('profile is', profile);
